...
# Script to launch spring-security-jee-pac4j-boot-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting spring-security-jee-pac4j-boot-demo..."
...
JAR="target/spring-security-jee-pac4j-boot-demo.jar"
if [ ! -f "$JAR" ]; then
...
if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 spring-security-jee-pac4j-boot-demo test completed successfully!"
    ...
    echo "💥 spring-security-jee-pac4j-boot-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
